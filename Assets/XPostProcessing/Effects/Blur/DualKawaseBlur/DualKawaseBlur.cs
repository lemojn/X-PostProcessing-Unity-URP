...
{
    [VolumeComponentMenu(VolumeMenu.Blur + "双重Kawase模糊 (Dual Kawase Blur)")]
    public class DualKawaseBlur : VolumeSettingBase
    {
    ...
    [VolumeRendererPriority(VolumePriority.Blur + 60)]
    public class DualKawaseBlurRenderer : VolumeRendererBase<DualKawaseBlur>
    {
        public override string ProfilerTag => "Blur-DualKawaseBlur";
        protected override string ShaderName => "Hidden/XPostProcessing/Blur/DualKawaseBlur";
        ...
        {
            m_BlitMaterial.SetFloat(ShaderIDs.BlurOffset, m_Settings.BlurRadius.value);
            ...
            {
                Blitter.BlitCameraTexture(cmd, m_TempLastRT, m_Pyramid[i].upRT, m_BlitMaterial, 1);
                m_TempLastRT = m_Pyramid[i].upRT;
            }
            // Render blurred texture in blend pass
            Blitter.BlitCameraTexture(cmd, m_TempLastRT, target, m_BlitMaterial, 1);
            m_TempLastRT = null;
        }
