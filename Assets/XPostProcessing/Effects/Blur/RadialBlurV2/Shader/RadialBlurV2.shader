Shader "Hidden/XPostProcessing/Blur/RadialBlurV2"
{
    HLSLINCLUDE
    ...
    half4 Frag_4Tap(Varyings i) : SV_Target
    {
        ...
        half scale = 1;
        half4 color = GetScreenColor(uv * scale + _RadialCenter); //1 MAD

        scale = _BlurRadius + 1;  //1 MAD
        color += GetScreenColor(uv * scale + _RadialCenter); //1 MAD
